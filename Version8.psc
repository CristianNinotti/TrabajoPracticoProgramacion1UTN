...
SubProceso cargaDatos(vueloDestino Por Referencia, filas, columnas, rutas, contadorRuta, capacidadPasajeros, r, cp)
	...
	Repetir
		...
		leer dni
		Si ConvertirANumero(dni) > 999999 y ConvertirANumero(dni)  < 9999999
			Escribir "Ingrese un dni con mas de 7 u 8 caracteres"
		FinSi
	Mientras Que ConvertirANumero(dni) > 999999 y ConvertirANumero(dni) < 9999999
	vueloDestino[contadorRuta,2] = dni
	
	Repetir
		escribir ("Ingrese telefono")
		leer tel
		Si ConvertirANumero(tel) < 999999 y ConvertirANumero(tel) > 999999999999
			Escribir "Ingrese un tel con mas de 6 digitos y menores a 12 digitos"
		FinSi
	Mientras Que ConvertirANumero(tel) < 999999 y ConvertirANumero(tel) > 999999999999
	...
	Repetir
		...
		Si  ConvertirANumero(numPasajeroFrec) <= 99 y ConvertirANumero(numPasajeroFrec) >= 999
